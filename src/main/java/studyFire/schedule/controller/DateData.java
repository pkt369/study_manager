...
public class DateData {
	...
	public Map<String, Integer> today_info(DateData dateData) {
		...
		int startDay = cal.getMinimum(Calendar.DATE); //달의 시작하는 날
		int endDay = cal.getActualMaximum(Calendar.DAY_OF_MONTH); //달의 끝나는 날
		int start = cal.get(Calendar.DAY_OF_WEEK);// 시작하는 요일
		
		Calendar todayCal = Calendar.getInstance();
		SimpleDateFormat ysdf = new SimpleDateFormat("yyyy");
		SimpleDateFormat msdf = new SimpleDateFormat("M");

		int today_year = Integer.parseInt(ysdf.format(todayCal.getTime())); //포멧으로 설정
		int today_month = Integer.parseInt(msdf.format(todayCal.getTime()));

		int search_year = Integer.parseInt(dateData.getYear());
		int search_month = Integer.parseInt(dateData.getMonth()) + 1; //todayCal.getTime의 Month는 0부터 시작

		int today = -1;
		if (today_year == search_year && today_month == search_month) { //찾는 년도와 오늘 년도, 이번달과 찾는 달이 같으면
